<x-admin.layout title="Create Coupon">
    <div class="mx-auto w-full max-w-4xl px-6 py-8 lg:px-8">
        <div class="flex items-center justify-between border-b border-zinc-200 pb-5">
            <div>
                ...
                <h1 class="mt-1 text-2xl font-bold text-zinc-950">Create New Coupon</h1>
                <p class="mt-1 text-xs text-zinc-600">Generate a custom promotional coupon code with specific discount values and constraints.</p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('admin.coupons.store') }}" class="mt-6 space-y-6 rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
            @csrf
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <div class="mt-1 flex gap-2">
                        <input
                            ...
                            required
                            class="admin-input mt-0 uppercase"
                        >
                        <button
                            type="button"
                            onclick="generateRandomCode()"
                            class="rounded-xl border border-zinc-200 bg-zinc-100 px-3 py-2 text-xs font-bold text-zinc-700 hover:bg-zinc-200"
                        >
                            Generate
                        </button>
                    </div>
                ...
                <div>
                    ...
                    <input
                        ...
                        id="title"
                        value="{{ old('title') }}"
                        placeholder="e.g. 20% Festival Celebration"
            ...
            <div>
                ...
                <textarea
                    ...
                    id="description"
                    rows="2"
                    placeholder="Short description of coupon terms and conditions..."
                    class="admin-input mt-1"
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="payment_method_eligibility" id="payment_method_eligibility" class="admin-input mt-1" required>
                        <option value="both" @selected(old('payment_method_eligibility', 'both') === 'both')>Both Online Payment & Cash On Delivery</option>
                        <option value="online" @selected(old('payment_method_eligibility') === 'online')>Online Payment Only</option>
                        <option value="cod" @selected(old('payment_method_eligibility') === 'cod')>Cash on Delivery Only</option>
                    </select>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 pt-2">
                ...
                <label for="is_active" class="text-xs font-bold text-zinc-800">
                    Active (Ready to be used immediately)
                </label>
            </div>
    ...
    <script>
        function generateRandomCode() {
            const prefixes = ['SPICE', 'FLAVOUR', 'HERB', 'OFFER', 'CHEF'];
