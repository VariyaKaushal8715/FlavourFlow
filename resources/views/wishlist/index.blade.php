    ...
    <section class="bg-zinc-950 py-10 text-white sm:py-14">
        <div class="mx-auto flex w-full max-w-7xl items-center justify-between gap-5 px-6 lg:px-8">
            <div>
                ...
                <h1 class="mt-2 text-3xl font-semibold sm:text-4xl">{{ __('ui.your_wishlist') }}</h1>
            </div>
            <a class="rounded-lg border border-white/25 px-4 py-2 text-sm font-semibold transition hover:bg-white hover:text-zinc-950" href="{{ route('home') }}#products">{{ __('ui.browse_products') }}</a>
        </div>
    </section>
    ...
    @else
        <section class="bg-zinc-50 py-20 sm:py-24">
            <div class="mx-auto max-w-xl px-6 text-center lg:px-8">
                ...
                <a class="mt-7 inline-flex rounded-lg bg-brand-primary px-5 py-3 text-sm font-semibold text-white transition hover:brightness-95" href="{{ route('home') }}#products">{{ __('ui.browse_products') }}</a>
            </div>
        </section>
    @endif
</x-site.layout>
